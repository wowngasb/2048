...
// Set up the game
GameManager.prototype.recover = function (previousState) {
...
GameManager.prototype.move = function (direction) {
  ...
  if (moved) {
    ...
    var obj = document.getElementById('next-op');
    var txt = ({0: 'up', 1: 'right', 2: 'down', 3: 'left'})[op];
    if(obj && txt) {
      obj.innerText = txt;
    }
    this.aiOp = op;
    // console.log(`after smartAI.nextMove: ${op}`);
...
GameManager.prototype.tryAI = function() {
  if (this.aiOp >= 0) {
    var op = this.aiOp;
    this.aiOp = -1;
    this.move(op);
...
GameManager.prototype.tryAuto = function(val) {
  var self = this;
  if (val) {
